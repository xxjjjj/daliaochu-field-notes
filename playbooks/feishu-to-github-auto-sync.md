...
打捞处的自动同步链路默认按一次触发完成一个最小闭环，不应让用户反复说“请继续”。
...
结构化摘要与业务转译
  ↓
判断 public_level / value_stage
  ↓
写入 notes / experiments / playbooks / risks
  ↓
git commit + push
  ↓
群内反馈文件路径、同步状态、下一步
...
1. 需要等待外部登录、授权、付费 Key、人工提供文件或业务样本。
2. 追源范围过大，单次无法完成全部仓库/论文/视频阅读。
3. 涉及敏感内容，必须先确认公开边界。
4. 需要执行有副作用动作，例如自动发布、批量修改、对外发送。

即便分阶段，也要先完成当前阶段可落地的事情：例如先写入“待追源”笔记、风险边界或实验计划，而不是只说“后面继续”。
...
- 本次已经写入或更新了哪个文件；
- public_level 和 value_stage；
- 是否已 commit/push；
- 下一步会沿哪条线继续追，不需要用户再补一句“请继续”。
