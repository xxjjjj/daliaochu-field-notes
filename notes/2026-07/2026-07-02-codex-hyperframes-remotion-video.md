...
**阶段补充：替代路线已有历史单镜头样片，当前环境复跑待验证。** 本文 `value_stage` 保留“可小实验”，因为 HyperFrames / Remotion 本体并未因此获得实测证明。交接证据见 [Issue #1](https://github.com/xxjjjj/daliaochu-field-notes/issues/1)。

本机私有项目的历史执行记录与用户确认记录显示，已跑通过“确定性 HTML 时间线 → Chrome 抽帧 → 本地真实 MiniMax / 海螺配音 → FFmpeg 合成”的单镜头路线。历史记录为 90 帧、5 fps，真实配音合成后的 MP4 为 17.64 秒、1280×720、H.264 视频与 AAC 单声道。2026-09-13 仅重新核对到样片文件仍存在，约 374 KiB；这些媒体参数来自历史记录，本轮没有重新解析、播放或合成验收。

实际失败边界也应保留：系统语音曾生成空音频，AVFoundation 编码失败；外挂 SRT 烧录因中文字体回退失败产生方块，最后采用 HTML 画面内字幕。历史依赖的临时 FFmpeg 二进制本轮已不存在，现有流程不能据此宣称今天可直接复跑。

这支持原研究的“可控的程序化视频值得做单镜头试验”判断；尚不能支持完整课程、长期批量稳定性、成本收益或业务上线，也不构成 Fish Audio / HeyGen 工具组合的验证。原始视频、配音、字幕与业务材料留在私有项目，公开只记录脱敏结论。

下一步在原项目复用已有脚本，恢复可用的 FFmpeg 入口，以虚构素材重跑一个镜头，检查画面内中文字幕、真实音轨和音画时长。该复跑是后续事项，本次交接不执行。

小马已读取基线笔记并完成[独立文档复核](https://github.com/xxjjjj/daliaochu-field-notes/issues/1#issuecomment-5652934518)，认可保留工具本体的实验阶段、明确替代路线与原工具组合的区别。回执因其 GitHub 评论认证不可用，由本机明确标注代转；小马未接收私有媒体，也未做媒体重测。此次跑通的是 GitHub 任务 → 小马复核 → 本机代转回执及回写，不是自动双向同步。
