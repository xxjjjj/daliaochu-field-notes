...
## 11. 实践回写 · 2026-09-13（更正）

**培训视频已完成整片，不止单镜头样片。** 先前回写只查到早期约 18 秒样片，就将其当成项目最终进度；用户指出遗漏后，本机补查最终产物，确认该判断错误。[Issue #1](https://github.com/xxjjjj/daliaochu-field-notes/issues/1) 保留交接与更正轨迹。

本轮直接读取完整 MP4 的电影头（mvhd），得到时长 **864.834 秒，即 14 分 24.834 秒**，文件大小 **22,244,191 字节**。最终合并清单列出 10 个批次，配套目录有完整成片及多处成片检查截图；用户明确确认培训视频已完成。本轮核验文件及容器时长，没有重新逐帧播放验收。

后续生产已从早期逐帧抽图转向 HTML 与音频实时录制：现有批次脚本通过 Chrome 取得页面录制结果，再用 FFmpeg 编码，最终合并成片。因此早期样片文档中的逐帧方案、失败记录和下一步计划不能代表最终交付状态。

这次实践支持“程序化视频路线已完成完整培训成片”，应作为已有成果沉淀，而不是再从单镜头试验开始。它仍不等于 HyperFrames / Remotion / Fish Audio / HeyGen 本体经过实测，因此本文针对这些工具的 `value_stage` 保留“可小实验”。原始视频、配音、字幕及业务材料继续留在私有项目。

原临时 FFmpeg 文件本轮不存在，只涉及未来重跑环境的恢复，不能据此否定已完成的成片，也不构成要求用户重做的理由。值得回写的是从逐帧到实时录制的路线调整、分批配音与合片经验；只有实际需要制作下一条视频时再核对运行依赖。

此前[小马文档复核回执](https://github.com/xxjjjj/daliaochu-field-notes/issues/1#issuecomment-5652934518)依据的是本机提供的不完整样片证据，没有接触私有成片，不能作为“项目仅完成样片”的独立佐证。遗漏及错误归纳由本机 Codex 负责。此次交接为 GitHub 任务→小马复核→本机代转回执，不代表自动同步或小马独立 GitHub 写入已配置。
