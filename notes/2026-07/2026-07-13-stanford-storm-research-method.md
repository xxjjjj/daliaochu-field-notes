...
截至 2026-09-13，STORM 式研究方法已做成本机 `storm-research` 技能，并实际使用，价值阶段更新为“已实践”。此前“下一步做成实验流程”的建议已被后续实践推进，不应继续写成仅安装或待尝试。

实际落地的是对研究方法的技能化：多视角问题生成、检索证据、结构化整理与人工校验。这与部署 Stanford STORM / Co-STORM 原始软件是不同的实践范围，不能相互替代证明。

## 11. 实践回写 · 2026-09-13

**来源事实：** 使用者在本次盘点中明确确认，已制作 STORM 技能并用过。本轮同时核对到本机 `storm-research` 技能文件，其中已包含研究问题界定、多视角发现、基于来源的迭代提问及证据整理流程。文件核对支持“方法已技能化”，实际使用事实来自使用者的一手确认，不能仅凭文件存在推断。

**当前结论：** 已完成“研究方法 → 可调用技能 → 实际使用”的实践。应回写这一进展，不再建议从头制作同一技能。

**证据边界与后续：** 本轮未逐项核对历史使用产物，没有足够证据评价覆盖面、引用质量、耗时或相较普通总结的改进幅度，也不声称复现了原论文指标。下一步优先从已有使用记录中整理一个脱敏案例，说明研究问题、技能实际执行过程、输出用途与使用者评价，再归纳效果及局限；无需为证明“用过”重做一遍研究。
