...
## 11. Fable 5 与 harness 的关系补充

后续追源到 Anthropic 官方 Fable 5 / Mythos 5 发布资料、Claude Fable 5 prompting 文档，以及第三方评测资料。更准确的区分是：

- Sutton 的 Bitter Lesson / Era of Experience 讲的是长期智能演进范式：搜索、学习、经验、环境反馈、可扩展计算。
- Fable 5 是一个具体模型产品，官方强调其长程自治、复杂任务、代码与知识工作能力更强。
- harness 不是 Fable 5 的唯一“成功原因”，而是让模型能力稳定落地的外部运行框架，包括任务定义、工具、上下文、记忆、预算、检查点、证据链、权限与回退机制。
- 很多社区文章会说 Fable 5 适合“优化你的 harness”，或者用更好的 harness 让弱一点的模型接近 Fable 5 的效果。这不等于 Fable 5 只是 harness，也不等于 Sutton 文章在直接解释 Fable 5 的工程实现。

可以把关系理解成三层：

1. 模型能力：Fable 5 自身更擅长长程探索、规划、执行和自我修正。
2. 运行框架：harness 把模型放进一个可控工作系统里，让它有文件状态、工具调用、检查点、验证机制和安全边界。
3. 学习机制：如果 harness 能记录经验、错误、反馈和复盘，再把这些回流到下一轮任务，才更接近 Era of Experience 所说的“运行中成长”。

因此，当前纠偏结论是：用户把“Agent 需要能在运行中成长”和“Fable 5 成功依赖 harness”联系起来，方向是对的，但两者不是同一层概念。Fable 5 的能力来自模型本身与训练/架构/安全体系；harness 是把这种能力变成可靠 Agent 工作流的工程承载；Sutton 的文章提供的是更底层的判断框架。
