...
截至 2026-09-13，Ponytail 已在日常编码中持续使用，不再只是待安装、待试用的对象。实践反馈暴露了一个关键局限：追求小改动时，Agent 可能不断针对表面现象加补丁，没有解决问题本身。当前应沉淀使用经验与失败案例，而不是重新安排安装试验；本文前述安装与 A/B 方案保留为历史建议，不代表本机尚未使用。

## 12. 实践回写 · 2026-09-13

**来源事实：** 使用者在本次盘点中明确确认 Ponytail 一直在用，并反馈会出现“不停打补丁，不去修问题本身”的情况。本轮会话也启用了 Ponytail full 模式。这里记录的是持续使用中的一手反馈，没有将其扩写成所有任务必然失败的结论。

**实践判断：** “最小改动”必须以找准根因为前提。若每次只压缩当前 diff，却在不同调用位置不断叠加例外，短期代码变动虽小，问题仍会反复出现。评价是否简化，应看问题是否被完整解决、是否需要反复返工，不能只看本次改了几行。

当前规则文本本来就强调先理解调用链、修根因；实际仍出现上述问题，说明写进规则不等于执行时可靠遵守。后续遇到同类案例时，应保留脱敏的症状、根因、原补丁与最终修复对照，并检查相关调用路径及能复现问题的验证。不能为了少改几行跳过必要的根因修复，也不因此扩展为无关重构。

**证据边界：** 本轮未逐案复核代码 diff，也没有对照实验来区分模型、任务上下文与 Ponytail 指令各自的影响；不能据此量化归因、发生率或实际节省成本。本文引用的外部基准不等于本机测量结果。价值阶段“已实践”表示已持续使用并得到反馈，不表示效果已全面验证。
